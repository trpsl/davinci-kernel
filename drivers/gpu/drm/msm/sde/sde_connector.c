...
int sde_connector_update_hbm(struct sde_connector *c_conn)
{
	...
	pr_debug("hbm_overlay:%d, mi_dimlayer_type:%d fod_hbm_enabled:%d\n", hbm_overlay,
	c_conn->mi_dimlayer_state.mi_dimlayer_type, dsi_display->panel->fod_dimlayer_hbm_enabled);
...
void sde_connector_fod_notify(struct drm_connector *conn)
{
	...
	if (dsi_display->panel->hbm_ntfy_skip_flag) {
		...
		pr_debug("%s: icon notify need skip hbm_ntfy_skip_flag[%d]", __func__, dsi_display->panel->hbm_ntfy_skip_flag);
		return;
	}
